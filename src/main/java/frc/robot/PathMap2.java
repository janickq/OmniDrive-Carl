...
import frc.robot.pathfinding.AStarAlgorithm;
import frc.robot.pathfinding.element.Grid;
import frc.robot.pathfinding.element.Tile;

public class PathMap2 {

  public final int ROW_COUNT = 45;  // x
  public final int COLUMN_COUNT = 90; //y
  public final double TILE_SIZE = 0.05;
  ...
  public void getPose(Pose2d startpoint, Pose2d endpoint) {
    curPose = startpoint;
    endPose = endpoint;
    Ai = Math.toIntExact(Math.round(((curPose.getTranslation().getX()) /TILE_SIZE)));
    Aj = Math.toIntExact(Math.round(((curPose.getTranslation().getY()) /TILE_SIZE)));
    Bi = Math.toIntExact(Math.round(((endPose.getTranslation().getX()) /TILE_SIZE)));
    Bj = Math.toIntExact(Math.round(((endPose.getTranslation().getY()) /TILE_SIZE)));
    start = new Tile(Ai, Aj);
    end = new Tile(Bi, Bj);
  }
